...
root_directory = root+"data/PHI/PHI_OHIO/" 
data_directory = root_directory+"data/"
filter_data = True
normalize_data = False

#ohio directories:
#ohio_data = 'OhioT1DM-2-training2020'
if sys.argv[-2] == 'test':
...
prediction_window = 6 #no. of future values to predict (6 denotes a prediction horizon of 5 * 6 = 30 min)
#prediction_window = 12 #no. of future values to predict (12 denotes a prediction horizon of 5 * 12 = 60 min)
skip = 0 #skip values in the prediction horizon
stride = history_window #stride for non-overlapping windows
imputation = 'linear' #other techniques : fourier
...
def xml_to_csv():
    subjs = glob.glob(data_directory + ohio_data + '/*.xml') #e.g. ../../../data/PHI/PHI_OHIO/data/OhioT1DM-training

    for subj_path in subjs:
        subj = subj_path.split('/')[-1] #e.g. Filename = 540-ws-training.xml

        subj = subj.split('-')[0] #e.g. Subject ID = 540
        doc = ET.parse(subj_path) #e.g. '../../../data/PHI/PHI_OHIO/data/OhioT1DM-training/540-ws-training.xml'
        ...
        for feature in features:
            ...
            for row in root[all_variables.index(feature)]: #e.g each entry in a given tag like 'glucose_level' is a dictionary with <date> and <value>
                ...
                if feature == 'temp_basal' or (feature == 'bolus' and signal[signal_keys[-2]] == 'square dual'):
                    start_date = datetime.datetime.strptime(signal[signal_keys[0]],'%d-%m-%Y %H:%M:%S')
                    end_date = datetime.datetime.strptime(signal[signal_keys[1]],'%d-%m-%Y %H:%M:%S')
                    elapsed_time = (end_date - start_date).total_seconds()/60.0 #number of minutes
                    raw_value = float(signal[(signal_keys[-1])])
                    ...
                    for i in range(len(list(interpolated_dates.values))):
                        if i == 0:
                            if feature == 'bolus':
                                bolus_type.append(signal[signal_keys[-2]]+' start')
                            value.append(0.00) #no amount absorbed at the first time instance
                        elif i < len(list(interpolated_dates.values)) - 1:
                            if feature == 'bolus':
                                bolus_type.append(signal[signal_keys[-2]])
                            value.append(rate)
                        else:
                            if feature == 'bolus':
                                bolus_type.append(signal[signal_keys[-2]]+' end')
                            value.append(rate)
                    ...
                    if feature == 'bolus':
                        bolus_type.append(signal[signal_keys[-2]])
                    dates.append(datetime.datetime.strptime(signal[signal_keys[0]],'%d-%m-%Y %H:%M:%S'))
                    value.append(signal[signal_keys[-1]])
        ...
        if not path.exists(data_directory + 'csv_files'): # ../../../../data/PHI/PHI_OHIO/data/csv_files/
        ...
        df.to_csv(data_directory + 'csv_files/' + ohio_data +'/'+subj+'.csv') #e.g. ../../../../data/PHI/PHI_OHIO/data/csv_files/OhioT1DM-training
...
# meals = 0 when not recorded, actual value when recorded. 
def impute_data():
    subjs = glob.glob(data_directory + 'csv_files/' + ohio_data + '/*.csv') #e.g. ../../../data/PHI/PHI_OHIO/data/csv_files/OhioT1DM-training
    for subj_path in subjs:
        
        subj = subj_path.split('/')[-1] #e.g. Filename = 540-ws-training.xml
        subj = subj.split('.')[0] #e.g. Subject ID = 540
        print('-------------Imputing data for Subject: '+subj+'-------------')
        original_df = pd.read_csv(subj_path,parse_dates=True,index_col=0)
        ...
        for i in range(len(indices)+1):
            ...
            if ohio_data == 'OhioT1DM-training':
                if sys.argv[-1] == '0' or sys.argv[-1] == '1':
                    ...
                    subset_df = subset_df[subset_df['glucose_level'].notna()] #imputation technique 2
        ...
        if not path.exists(data_directory + 'csv_files/' + ohio_data + '/imputed/' + seg): #e.g. ../../../data/PHI/PHI_OHIO/data/csv_files/OhioT1DM-training/imputed
            os.mkdir(data_directory + 'csv_files/' + ohio_data + '/imputed/' + seg)
        ...
        df.to_csv(data_directory + 'csv_files/' + ohio_data + '/imputed/'+ seg + subj +'.csv')
    
    with open(data_directory + 'csv_files/' + ohio_data + '/imputed/' + seg + 'multivariate_continuous_subsets.pickle', 'wb') as f:
...
def get_windowed_data():
    pickledDictionary = glob.glob(os.path.join(data_directory + 'csv_files/' + ohio_data + '/imputed/' + seg,"multivariate_continuous_subsets.pickle"))

    with open(pickledDictionary[0], 'rb') as f:
        unpickled_data = pickle.load(f, encoding='latin1')
    
    subjs_paths = glob.glob(data_directory + 'csv_files/' + ohio_data + '/imputed/' + seg + '*.csv') #e.g. ../../../data/PHI/PHI_OHIO/data/csv_files/OhioT1DM-training/imputed
    subjs_paths.sort()
    all_windowed_data = {}
    for subj_data_file in subjs_paths:   
        subj = subj_data_file.split('/')[-1] #e.g. Filename = 540-ws-training.csv
        subj = subj.split('.')[0] #e.g. Subject ID = 540
    ...
    with open(data_directory + 'csv_files/' + ohio_data + '/imputed/'+ seg + output_file , 'wb') as f:
        # Pickle the 'all_subjects_data' dictionary using the highest protocol available.
        pickle.dump(all_windowed_data, f, pickle.HIGHEST_PROTOCOL)

if __name__ == "__main__":
    if len(sys.argv) > 1:
        prediction_window = int(sys.argv[2])
        if sys.argv[1] == 'xml_to_csv':
            xml_to_csv()
        elif sys.argv[1] == 'impute_data':
            impute_data()
        elif sys.argv[1] == 'get_windowed_data':
            get_windowed_data()
        elif sys.argv[1] == 'all':
            #xml_to_csv()
            impute_data()
            ...
            print("Function list: xml_to_csv(), impute_data(), get_windowed_data")
    else:
        print("Function list: xml_to_csv(), impute_data(), get_windowed_data(). Usage: python preprocess_ohio.py <function> <PH> <train_or_test> <ablation+code> e.g. python preprocess_ohio.py xml_to_csv PH test 0")
        exit(-1)
